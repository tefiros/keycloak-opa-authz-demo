...
public class OpaAccessPolicyProvider implements AccessPolicyProvider {
    ...
    public enum Option {
        ...
        REQUEST_BODY("request-body", ProviderConfigProperty.BOOLEAN_TYPE, "Request Body", "If enabled, the request body will be sent with OPA requests.", "true"), //
    ...
    protected OpaRequestContext createRequestContext(KeycloakSession session, ConfigWrapper config) {
        ...
        if (config.getBoolean(Option.REQUEST_BODY.key, true)) {
            builder.body(extractRequestBody(session));
        }
    ...
    protected String extractRequestBody(KeycloakSession session) {
        ...
        if (request != null) {
            try {
                ...
                for (Map.Entry<String, List<String>> entry : formParameters.entrySet()) {
                    ...
                    baos.write((key + "=" + value + "&").getBytes(StandardCharsets.UTF_8));
                }
            ...
            System.out.println("Request is null or has no input stream");
        }
        return null;
    }
